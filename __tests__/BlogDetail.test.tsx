...
beforeAll(() => {
  server.listen() //サーバーの開始
})
afterEach(() => {
  server.resetHandlers() //テスト終了後にリセット
  cleanup()
})
afterAll(() => {
  server.close() //サーバーの停止
})

describe(`Blog detail page`, () => {
  it('Should render datailed content of ID 1', async () => {
  ...
  it('Should render datailed content of ID 2', async () => {
    const { page } = await getPage({
      route: '/posts/2',
    })
    render(page)
